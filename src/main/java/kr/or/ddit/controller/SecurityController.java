...
public class SecurityController {
	 ...
	 * - 스프링 시큐리티 필터 기반으로 동작하기 때문에 스프링 MVC와 분리되어 동작한다.
	 * 
	 * 		# 기본 보안 기능
	 * 		
	 * 			- 인증(Authentication)
	 ...
	 * 			외도한 행위(수정, 삭제 , 등록 등등) 를 특정 웹사이트에 요청하게 하는 공격을 말한다.
	 * 	
	 * 				> CSRF 공격을 대비하기 위해서는 스프링 시큐리티 CSRF Token을 이용하여 인증을 진행한다.
	 * 
	 * 		# 시큐리티 인증 구조
	 * 
	 * 			클라이언트에서 타겟으로 들어가기 위해 요청을 진행한다. 이때, 타겟에 설정되어 있는 요청 접근 권한이 '사용자' 등급일떄로 설정되어 있다고 가정하자.
	 ...
	 * 			클라이언트에서 서버로 요청한 HttpServletRequest 의 요청 객체를 AuthenticationFilter가 요청을 Intercept한다.
	 * 			UsernamePasswordAuthenticationToken을 통해 인증을 진행 할 토튼을 만들어  AuthenticationManager에게 위임한다.
	 * 			넘겨 받은 id,pw 를 이용해 인증을 진행하는데 성공 시, Authentication 객체 생성과 성공을 전달하고, 그렇지 않으면 Exception 에러를 발생시킨다.
	 * 			인증에 성공 후, 만들어진 Authentication 객체를 AuthenticationProvider에게 전달하고 UserDetailService에서 넘겨 받은
	 ...
	 * 			그리고 최종 , 쿠키로 만들어진 JSESSIONID가 유효한지를 검증 후 유효하면 인증을 완료하여 타겟으로 진입을 돕는다.
	 * 				
	 * 			HttpServletRequest >> AuthenticationFilter >> Authentication >> AuthenticationProvider >> UserDetailService >>
	 * 			Database  >> UserDetails >> User >>  User Session >> SecurityContextHolder (UserDetail) >> JSESSIONID 유효성 검사
	 * 
	 * 
	 * 	2. 스프링 시큐리티 설정
	 * 
	 ...
	 * 				> 스프링 시큐리티는 필터 기반이므로 필터 체인을 등록한다.
	 * 					> context-param 태그의 param-value 추가(추가 파라미터 : /WEB-INF/spring/security-context.xml)
	 ...
	 * 					대상 			|			화면			| 	접근정책
	 * 			──────────────────────────────────────────────────────────────────────────
	 * 				일반 게시판 		|	목록 화면 				|	모두가 접근이 가능하다
	 * 								|	등록 화면 				|	로그인한 회원만 접근이 가능하다
	 * 			──────────────────────────────────────────────────────────────────────────
	 * 				공지사항 			|	목록 화면 				|	모두가 접근이 가능하다
	 ...
	 * 
	 *  3. 접근 제한 설정
	 *  - 시큐리티 설정을 통해 특정 URI에 대한 접근을 제한할 수 있다.
	 *  
	 *  		# 환경 설정
	 *  		
	 *  			- 스프링 시큐리티 설정
	 ...
	 *  			- 공지사항 게시판 목록 화면 (모두가 접근 가능하도록 되어있다 : permitAll)
	 ...
	 *   			> 관리자 등급인 admin 계정은 ROLE_MEMBER 권한도 가지고 있는 계정이므로 페이지로 접근 가능
	 *  		- 공지사항 게시판 등록 화면
	 *  			> 접근 제한에 걸려 스프링 시큐리티가 기본적으로 제공하는 로그인 페이지로 연결되고, 
	 ...
	 *  
	 *  		이 부분은 꼭 설정해야하는게 아니라, 패키지 안에서 사용되고 있는 여러가지 컨트롤러와 자원들을 빈으로 증록해야 하는 구조인 경우
	 ...
	 *  		- 공지사항 게시판 등록 페이지
	 *  			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고, 회원 권한을 가진 계정으로 접근 시에 공지사항 게시판 등록 페이지는
	 ...
	 *  				> AccessDeniedHandler 인터페이스를 참조 받아서 handle 메소드를 재정의하여 사용합니다.
	 *  				: 우리는 접근이 거부되었을 떄 빈으로 등록해둔 CustomAccessDeniedHandler 클래스가 발동해 메소드가 실행되고
	 *  				response 내장 객체를 활용하여 /accessError URL로 이동하여 접근 거부시 보여질 페이지로 이동하지만 이곳에서 더 많은 로직을 처리하길 원하면
	 *  				request, response 내장 객체를 이용하여 다양한 처리가 가능케 된다.
	 *  
	 *  	# 화면 설정
	 *  		- 일반 게시판 등록 페이지
	 *  			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고, 회원 권한을 가진 계정으로 접근 시 접근 가능
	 *  		- 공지사항 게시판 등록 페이지
	 *  			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고, 회원 권한을 가진 계정으로 접근 시에 공지사항 게시판 등록 페이지는
	 *  			관리자 권한만 접근 가능하므로 접근이 거부된다.
	 *  			이떄, access-denied-handler로 설정되어 있는 ref 속성에 부여된 사용자가 정의한 접근 거부 처리자에 해당하는 컨트롤러 메소드로 이동하고
	 ...
	 *  				: 사용자가 직접 만든 로그인 페이지로 이동할 '/login' URL을 가지고 있는 컨트롤러 메소드를 정의한다.
	 *  
	 *  	# 로그인 페이지 정의
	 *  		- 사용자가 정의한 로그인 컨트롤러
	 ...
	 *  		- 로그인 성공 처리자 2
	 *  			> AuthenticationSuccessHandler 인터페이스를 직접 구현하여 인증 전에 접근을 시도한 URL로 리다이렉트하는 기능을 구현한다.
	 *  
	 *  	# 화면 설명
	 *  		- 일반 게시판 등록 화면
	 *  			> 사용자가 정의한 로그인 페이지에서 회원 권한에 해당하는 계정으로 로그인 시, 성공했다면 성공 처리자인 CustomLoginSuccess 클래스로 넘어가
	 *  			넘겨받은 파라미터들 중 authentication안에 principal로 User 정보를 받아서 username 과  password를 출력한다.
	 ...
	 *  - 지정한 형식으로 테이블을 생성하면 JDBC를 이용해서 인증/ 인가 처리 할 수 있다.
	 ...
	 *  			> 기존 데이터베이스 연결을 위한 라이브러리가 추가되어 있다.(SecurityTest 프로젝트를 사용할 경우 등록 진행)
	 *  		** 우리는 기존 셋팅 시, 추가를 먼저하고 옴
	 *  
	 ...
	 *  			> security:authenticaton-manager > 태그 설정
	 *  
	 *  	# 비밀번호 암호화 처리기 클래스 정의
	 *  		- 비밀번호 암호화 처리기
	 *  			> 스프링 시큐리티 5부터는 기본적으로 PasswordEncoder를 지정해야 하는데, 제대로 하려면 생성된 사용자 테이블(users)에 비밀번호를 암호화하여 저장해야 한다.
	 *  			테스트를 위해서 생성한 데이터는 암호화를 처리하지 않으므로 로그인 하면 당연히 로그인 에러가 발생할 것이다.( 암호화된 비밀번호가 날라가는게 아니라서)
	 *  			그래서 암호화를 하지 않는 PasswordEncoder를 직접 구현하여 지정하면 로그인 시 암호화를 고려하지 않으므로 로그인이 정상적으로 이루어지는걸 확인할 수 있다.
	 *  		- security/CustomNoOpPasswordEncoder 클래스 구현 
	 *  
	 *  		
	 *  
	 *  11. 사용자 테이블 이용한 인증/인가 처리
	 *  - 스프링 시큐리티가 기본적으로 이용하는 테이블 구조를 그대로 생성해서 사용해도 되지만 기존에 구축된 회운 테이블이 있다면 약간의 작업으로 기존 테이블을 활용할 수 있다.
	 *  
	 *  	# 데이터베이스 테이블 준비
	 *  		- member, member_auth 테이블 준비
	 *  
	 *  	# 환경 설정
	 *  		- 스프링 시큐리지 설정(security-context.xml 설정)
	 ...
	 *  
	 *  		** BcrytPasswordEncoder 클래스를 활용한 단방향 비밀번호 암호화
	 *  			> encode() 메서드를 통해서 SHA-2 방식의 8바이트 Hash 암호를 매번 랜덤하게 생성합니다.
	 *  			> 똑같은 비밀번홀를 입력하더라도 암호화되는 문자열은 매번 다른 문자열을 반환한다.
	 *  
	 *  			비밀번호를 입력하면 암호화된 비밀번호로 인코딩되는데, 암호화된 비밀번호와 디비 테이블에 있는 암호화된 비밀번호가 일치한지를 파악 한 후 일치하면 로그인
	 *  			성공으로 다음 스탭을 진행한다.
	 ...
	 *  				> 기존 사용중인 read를 기반으로 한 readById Mapper 재정의
	 ...
	 *				> 여러 롤들 중에서 하나라도 해당하는 롤리 있으면 true
	 *			- principal
	 *				> 인증된 사용자의 사용자 정보(UserDetails 인터페이스를 구현한 클래스의 객체)를 의미
	 *			- authentication
	 *				> 인증된 사용자의 인증 정보(Authentication 인터페이스를 구현한 클래스의 객채)를 의미
	 *			- permitAll
	 *				> 모든 사용자에게 허용
	 *			- denyAll
	 *				> 모든 사용자에게 거부
	 ...
	 *			- isAuthenticated()
	 *				> 인증된 사용자면 true
	 *			- isFulltAuthenticated()
	 *				> Remember-me 로 인증된 것이 아닌 일반적인 방법으로 인증된 사용자인 경우 true
	 *
	 *		# 표현식 사용
	 *			- 표현식을 이용하여 동적 화면 구성
	 *				> home.jsp 수정
	 ...
	 *  
	 *  	# 환경 설정
	 *  		> <security:remember-me data-source-ref="dataSource" token-validity-secons="604800"/> 태그설정
	 *  		> security:logout logout-url="logout" invalidate-session="true" delete-cookies="remember-me, JSESSION_ID/> 태그 설정
	 *  		
	 *  	# 자동 로그인
	 *  		- 로그인 상태 유지 체크박스 추가
	 ...
	 *  		- JSESSIOND, remember-me 쿠키가 만들어진다.
	 *  		- JSESSIOND를 삭제 후, 다시 로그인을 진행하더라도 로그인 후 진행될 페이지가 정상적으로 나타나는 걸 확인할 수 있다.
